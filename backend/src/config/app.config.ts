...
import {
  IsEnum, IsInt, IsOptional, IsString, IsUrl, Max, Min,
...
class EnvValidator {
  ...
  @IsInt() @Min(0) @Max(65535) @IsOptional() @Type(() => Number) APP_PORT!: number;
  ...
  @IsInt() @Min(0) @IsOptional() @Type(() => Number) EXTERNAL_API_TIMEOUT!: number;
  @IsInt() @Min(0) @IsOptional() @Type(() => Number) AFFILIATE_API_TIMEOUT!: number;
}
...
export default registerAs<AppConfig>('app', () => {
  ...
  const frontendDomain =
      process.env.FRONTEND_DOMAIN || (isDevelopment ? 'http://localhost:3000' : 'https://dev.uniaffcrm.com');

  const backendDomain  =
      process.env.BACKEND_DOMAIN  || (isDevelopment ? 'http://localhost:3001' : 'https://dev.uniaffcrm.com/api');

  return {
    nodeEnv, isDevelopment, isProduction, isTest,
    name: process.env.APP_NAME || 'Hermes CRM Backend',
    workingDirectory: process.env.PWD || process.cwd(),
    frontendDomain, backendDomain, port,
    ...
    externalApis: {
      leads: {
        ...
        timeout: process.env.EXTERNAL_API_TIMEOUT ? parseInt(process.env.EXTERNAL_API_TIMEOUT, 10) : 5000,
      },
      products: {
      ...
      affiliate: {
        ...
        apiKey:  process.env.EXTERNAL_API_KEY!,
        timeout: process.env.AFFILIATE_API_TIMEOUT ? parseInt(process.env.AFFILIATE_API_TIMEOUT, 10) : 5000,
      },
    },
  };
});
