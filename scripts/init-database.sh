...
check_database() {
    ...
    PGPASSWORD=$DATABASE_PASSWORD pg_isready -h $DB_HOST -p $DB_PORT -U postgres
    return $?
}
...
check_database_exists() {
    ...
    PGPASSWORD=$DATABASE_PASSWORD psql -h $DB_HOST -p $DB_PORT -U postgres -d postgres -c "SELECT 1 FROM pg_database WHERE datname='hermes';" | grep -q 1
    return $?
}
...
create_database() {
    ...
    PGPASSWORD=$DATABASE_PASSWORD psql -h $DB_HOST -p $DB_PORT -U postgres -d postgres -c "CREATE DATABASE hermes;" || echo "Database might already exist"
}
