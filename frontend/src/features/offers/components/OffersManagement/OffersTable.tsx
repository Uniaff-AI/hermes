...
const OffersTable: FC<OffersTableProps> = ({ searchQuery, filters = {} }) => {
    ...
    const filteredOffers = useMemo(() => {
        ...
        if (filters.vertical) {
            filtered = filtered.filter((offer) => offer.vertical === filters.vertical);
        }
        ...
        return (
            <div className="bg-white p-8 rounded shadow">
                <div className="text-center text-red-500">
                    Ошибка загрузки офферов: {error instanceof Error ? error.message : 'Неизвестная ошибка'}
                </div>
            </div>
        );
    }
                ...
                <tbody>
                    {filteredOffers.length > 0 ? (
                        filteredOffers.map((offer, index) => (
                            <tr key={`${offer.productId}-${index}`} className="border-b hover:bg-gray-50 transition">
