...
const OffersFilters: FC<OffersFiltersProps> = ({ value, onChange }) => {
    const [isAdvancedVisible, setAdvancedVisible] = useState(false);

    const filterOptions = useMemo(
        () => ({
            status: ['Все', 'Активные', 'Неактивные', 'В ожидании'],
            category: ['Все категории', 'Финансы', 'Игры', 'Электроника', 'Одежда'],
            group: ['Все группы', 'Группа A', 'Группа B', 'Группа C'],
            country: ['Все страны', 'Россия', 'США', 'Германия', 'Франция'],
            payment: ['Все выплаты', 'Высокие', 'Средние', 'Низкие'],
        }),
        []
    );
                ...
                {isAdvancedVisible && (
                    ...
                    >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 pt-6">
                            {/* Column 1 */}
                            <div>
                                <FilterSelect label="Статус" color="bg-green-500" options={filterOptions.status} />
                                <FilterSelect label="Категория" color="bg-purple-500" options={filterOptions.category} />
                                <FilterSelect label="Группа" color="bg-red-500" options={filterOptions.group} />
                            </div>
                            {/* Column 2 */}
                            <div>
                                <FilterSelect label="Страна" color="bg-blue-500" options={filterOptions.country} />
                                <FilterSelect label="Выплата" color="bg-orange-500" options={filterOptions.payment} />
                            </div>
                        </div>
                    </motion.div>
                )}
